import { r as rootContext } from "./root-context-CaKowcmF.js";
const emptyOptions = {};
      ...
      if (events[index2][0] === "enter" && events[index2][1].type === "strikethroughSequenceTemporary" && events[index2][1]._close) {
        ...
        while (open--) {
          ...
          events[index2][1].end.offset - events[index2][1].start.offset === events[open][1].end.offset - events[open][1].start.offset) {
            ...
            splice(nextEvents, nextEvents.length, 0, [["exit", text2, context], ["enter", events[index2][1], context], ["exit", events[index2][1], context], ["exit", strikethrough, context]]);
            splice(events, open - 1, index2 - open + 3, nextEvents);
...
function castFactory(testFunction) {
  return check;
  function check(value2, index2, parent2) {
    return Boolean(
...
function zwitch(key, options) {
  ...
  function one2(value2, ...parameters) {
    ...
    if (value2 && own$2.call(value2, key)) {
...
function code(node2, _, state, info) {
  ...
  if (formatCodeAsIndented(node2, state)) {
    ...
    const value3 = state.indentLines(raw, map$1);
    exit3();
    return value3;
  }
  const tracker = state.createTracker(info);
...
function listItem$1(node2, parent2, state, info) {
...
function joinDefaults(left, right, parent2, state) {
  if (right.type === "code" && formatCodeAsIndented(right, state) && (left.type === "list" || left.type === right.type && formatCodeAsIndented(left, state))) {
    return false;
  }
  if ("spread" in parent2 && typeof parent2.spread === "boolean") {
    if (left.type === "paragraph" && // Two paragraphs.
    (left.type === right.type || right.type === "definition" || // Paragraph followed by a setext heading.
    right.type === "heading" && formatHeadingAsSetext(right, state))) {
      return;
    }
    return parent2.spread ? 1 : 0;
  }
}
...
function containerPhrasing(parent2, state, info) {
  ...
  while (++index2 < children.length) {
    ...
    if (index2 + 1 < children.length) {
      ...
      after = handle2 ? handle2(children[index2 + 1], parent2, state, {
        before: "",
        after: "",
        ...tracker.current()
      }).charAt(0) : "";
    ...
    let value2 = state.handle(child, parent2, state, {
      ...tracker.current(),
      after,
      before
    });
...
function containerFlow(parent2, state, info) {
  const indexStack = state.indexStack;
  const children = parent2.children || [];
  const tracker = state.createTracker(info);
  const results = [];
  ...
  while (++index2 < children.length) {
    ...
    results.push(
      tracker.move(
        state.handle(child, parent2, state, {
          before: "\n",
          after: "\n",
          ...tracker.current()
        })
    ...
    if (index2 < children.length - 1) {
      results.push(
        tracker.move(between(child, children[index2 + 1], parent2, state))
      );
    }
  }
  indexStack.pop();
...
function between(left, right, parent2, state) {
  let index2 = state.join.length;
  while (index2--) {
    const result = state.join[index2](left, right, parent2, state);
...
function containerPhrasingBound(parent2, info) {
  return containerPhrasing(parent2, this, info);
}
function containerFlowBound(parent2, info) {
  return containerFlow(parent2, this, info);
}
...
function gfmTableToMarkdown(options) {
  ...
  function inlineCodeWithTable(node2, parent2, state) {
    let value2 = handle.inlineCode(node2, parent2, state);
...
function listItemWithTaskListItem(node2, parent2, state, info) {
  ...
  let value2 = handle.listItem(node2, parent2, state, {
    ...info,
    ...tracker.current()
  });
  if (checkable) {
...
const isScalarValue = (value2) => !value2 || typeof value2 !== "function" && typeof value2 !== "object";
...
${indent2}:`;
  ...
  const valueStr = stringify$1(value2, ctx, () => valueCommentDone = true, () => chompKeep = true);
  let ws = " ";
...
${ctx.indent}`;
    ...
    const flow2 = ctx.inFlow ?? value2.flow ?? value2.items.length === 0;
    if (hasNewline || !flow2) {
...
const intIdentify$2 = (value2) => typeof value2 === "bigint" || Number.isInteger(value2);
...
const stringifyJSON = ({ value: value2 }) => JSON.stringify(value2);
const jsonScalars = [
  {
    identify: (value2) => typeof value2 === "string",
    default: true,
...
const timestamp = {
  ...
  stringify: ({ value: value2 }) => (value2 == null ? void 0 : value2.toISOString().replace(/(T00:00:00)?\.000Z$/, "")) ?? ""
};
const schema = [
  map,
  seq,
  ...
  for (let i = 0; i < fc.items.length; ++i) {
    ...
    if (!isMap2 && !sep && !props.found) {
      ...
      if (valueProps.found) {
        ...
        if ("source" in value2 && ((_a = value2.source) == null ? void 0 : _a[0]) === ":")
...
function resolveBlockScalar(ctx, scalar, onError) {
  ...
  for (let i = 0; i < chompStart; ++i) {
    const [indent2, content2] = lines[i];
    if (content2 === "" || content2 === "\r") {
    ...
    offset += indent2.length + content2.length + 1;
  }
  ...
  for (let i = contentStart; i < chompStart; ++i) {
    ...
    if (type === Scalar.BLOCK_LITERAL) {
      value2 += sep + indent2.slice(trimIndent) + content2;
      sep = "\n";
...
function findScalarTagByTest({ atKey, directives, schema: schema2 }, value2, token, onError) {
  const tag = schema2.tags.find((tag2) => {
    var _a;
    return (tag2.default === true || atKey && tag2.default === "key") && ((_a = tag2.test) == null ? void 0 : _a.test(value2));
  }) || schema2[SCALAR$1];
  if (schema2.compat) {
...
const PHRASING = /* @__PURE__ */ new Set([
  "text",
  "strong",
  "emphasis",
  "delete",
...
function isRepresentablePhrasing(nodes) {
  return (nodes || []).every((node2) => {
    if (!PHRASING.has(node2.type)) {
      return false;
    }
    const children = node2.children;
...
function isRepresentableListItem(item) {
  ...
  if (!isRepresentablePhrasing(children[0].children)) {
    return false;
  }
  return item.checked === null || item.checked === void 0;
}
function isRepresentable(node2) {
  switch (node2.type) {
    case "paragraph":
      return !SHORTCODE.test(plainText(node2).trim()) && isRepresentablePhrasing(node2.children);
    case "heading":
      return isRepresentablePhrasing(node2.children);
    case "code":
      return true;
    case "table": {
      const rows = node2.children || [];
      return rows.every((row) => (row.children || []).every(
        (cell) => isRepresentablePhrasing(cell.children)
      ));
    }
    case "blockquote": {
      const children = node2.children || [];
      return children.length === 1 && children[0].type === "paragraph" && isRepresentablePhrasing(children[0].children);
    }
    case "list": {
      const children = node2.children || [];
      return children.length > 0 && children.every(
        (item) => item.type === "listItem" && isRepresentableListItem(item)
      );
    }
    default:
      return false;
...
function parseMarkdown(source) {
  ...
  for (const node2 of tree.children) {
    ...
    const start = position2.start.offset;
    const end = position2.end.offset;
    if (node2.type === "yaml") {
      const raw = source.slice(start, end);
      let data = null;
      let valid = true;
      try {
        data = parse(String(node2.value ?? ""));
      } catch {
        data = null;
        valid = false;
      }
    ...
    blocks.push({
      ...
      editable: isRepresentable(node2)
    });
  }
  return { source, frontmatter: front, blocks };
}
const SOURCE_ATTRIBUTE = "data-ct-md";
const LANGUAGE_ATTRIBUTE = "data-ct-lang";
function escapeText(value2) {
  return value2.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}
function escapeAttribute(value2) {
  return escapeText(value2).replace(/"/g, "&quot;");
}
function attr(name, value2) {
  if (value2 === null || value2 === void 0 || value2 === "") {
    return "";
  }
  return ` ${name}="${escapeAttribute(String(value2))}"`;
}
function inline(nodes) {
  return nodes.map(inlineOne).join("");
}
function inlineOne(node2) {
  switch (node2.type) {
    case "text":
      return escapeText(String(node2.value ?? ""));
    case "strong":
      return `<b>${inline(node2.children)}</b>`;
    case "emphasis":
      return `<i>${inline(node2.children)}</i>`;
    case "delete":
      return `<del>${inline(node2.children)}</del>`;
    case "inlineCode":
      return `<code>${escapeText(String(node2.value ?? ""))}</code>`;
    case "break":
      return "<br>";
    case "link":
      return `<a${attr("href", node2.url)}${attr("title", node2.title)}>${inline(node2.children)}</a>`;
    case "image":
      return `<img${attr("src", node2.url)}${attr("alt", node2.alt ?? "")}${attr("title", node2.title)}>`;
    default:
      return escapeText(String(node2.value ?? ""));
  }
}
function listItem(item) {
  const children = item.children || [];
  const text2 = inline(children[0].children);
  const nested = children[1] ? block(children[1]) : "";
  return `<li>${text2}${nested}</li>`;
}
function block(node2) {
  switch (node2.type) {
    case "paragraph":
      return `<p>${inline(node2.children)}</p>`;
    case "heading": {
      const depth = Math.min(Math.max(Number(node2.depth) || 1, 1), 6);
      return `<h${depth}>${inline(node2.children)}</h${depth}>`;
    }
    case "blockquote": {
      const paragraph2 = node2.children[0];
      return `<blockquote>${inline(paragraph2.children)}</blockquote>`;
    }
    case "code":
      return `<pre${attr(LANGUAGE_ATTRIBUTE, node2.lang)}>${escapeText(String(node2.value ?? ""))}</pre>`;
    case "list": {
      const tag = node2.ordered ? "ol" : "ul";
      const start = node2.ordered && node2.start !== null && node2.start !== 1 ? attr("start", node2.start) : "";
      const items = (node2.children || []).map((item) => listItem(item)).join("");
...
function table(node2) {
  const align = node2.align || [];
  const rows = node2.children || [];
  const cells = (row, tag) => (row.children || []).map((cell, column) => `<${tag}${attr("align", align[column])}>${inline(cell.children)}</${tag}>`).join("");
  const head = rows.length ? `<thead><tr>${cells(rows[0], "th")}</tr></thead>` : "";
  const body = rows.slice(1).map((row) => `<tr>${cells(row, "td")}</tr>`).join("");
  return `<table>${head}${body ? `<tbody>${body}</tbody>` : ""}</table>`;
}
function staticBlock(source) {
  return `<div data-ce-tag="static" class="ct-md-static"><pre>${escapeText(source)}</pre></div>`;
}
function blockToHTML(entry, source) {
  const html2 = entry.editable ? block(entry.node) : staticBlock(source.slice(entry.start, entry.end));
  const tagEnd = html2.indexOf(">");
  if (tagEnd === -1) {
    return html2;
  }
  const selfClosing = html2[tagEnd - 1] === "/";
  const insertAt = selfClosing ? tagEnd - 1 : tagEnd;
  return html2.slice(0, insertAt) + attr(SOURCE_ATTRIBUTE, entry.index) + html2.slice(insertAt);
}
function toHTML(blocks, source) {
  return blocks.map((entry) => blockToHTML(entry, source)).join("");
}
function text(value2) {
  return { type: "text", value: value2 };
}
function parent(type, children, extra) {
...
function normalise(nodes) {
  for (let i = 0; i < nodes.length; i += 1) {
    ...
    let value2 = String(node2.value).replace(WHITESPACE$1, " ");
    if (i === 0) {
      value2 = value2.replace(/^ /, "");
    }
    if (i === nodes.length - 1) {
...
function mergeText(nodes) {
  ...
  for (const node2 of nodes) {
    ...
    if (node2.type === "text" && last && last.type === "text") {
      last.value = String(last.value) + String(node2.value);
    } else {
      out.push(node2);
    }
  }
...
function collectInline(node2, out) {
  if (node2.nodeType === 3) {
    out.push(text(node2.nodeValue || ""));
    return;
  }
  if (node2.nodeType !== 1) {
    return;
  ...
  switch (tag) {
    case "b":
    case "strong":
      out.push(parent("strong", inlineChildren(element2)));
      return;
    case "i":
    case "em":
      out.push(parent("emphasis", inlineChildren(element2)));
      return;
    case "del":
    case "s":
    case "strike":
      out.push(parent("delete", inlineChildren(element2)));
      return;
    case "code":
      out.push({ type: "inlineCode", value: element2.textContent || "" });
      return;
    case "br":
      out.push({ type: "break" });
      return;
    case "a":
      out.push(parent("link", inlineChildren(element2), {
        url: element2.getAttribute("href") || "",
        title: element2.getAttribute("title") || null
      }));
      return;
    case "img":
      out.push({
...
function listItems(element2) {
  const items = [];
  for (const li of Array.from(element2.children)) {
    ...
    const children = [parent("paragraph", normalise(mergeText(inlineNodes)))];
    if (nested) {
...
function listFrom(element2) {
  const ordered = element2.tagName.toLowerCase() === "ol";
  const start = element2.getAttribute("start");
  return parent("list", listItems(element2), {
    ordered,
    start: ordered ? start ? Number(start) : 1 : null,
    spread: false
  });
}
function tableFrom(element2) {
  const rows = [];
  const align = [];
  for (const tr of Array.from(element2.querySelectorAll("tr"))) {
    const cells = [];
...
function blockFrom(element2) {
  const tag = element2.tagName.toLowerCase();
  if (/^h[1-6]$/.test(tag)) {
    return parent("heading", inlineChildren(element2), {
      depth: Number(tag.slice(1))
    });
  }
  switch (tag) {
    case "p":
      return parent("paragraph", inlineChildren(element2));
    case "blockquote":
      return parent("blockquote", [parent("paragraph", inlineChildren(element2))]);
    case "pre":
      return {
        type: "code",
        lang: element2.getAttribute(LANGUAGE_ATTRIBUTE) || null,
        meta: null,
        value: element2.textContent || ""
      };
    case "ul":
    case "ol":
      return listFrom(element2);
    case "table":
      return tableFrom(element2);
    case "img":
...
function fromHTML(html2) {
  const sandbox = rootContext().createSandboxDocument();
  const wrapper = sandbox.createElement("div");
  wrapper.innerHTML = html2;
  const blocks = [];
  const claimed = /* @__PURE__ */ new Set();
  for (const element2 of Array.from(wrapper.children)) {
    const marker = element2.getAttribute(SOURCE_ATTRIBUTE);
    let index2 = marker === null ? null : Number(marker);
    if (index2 !== null && (Number.isNaN(index2) || claimed.has(index2))) {
      index2 = null;
    }
    if (index2 !== null) {
      claimed.add(index2);
    }
    if (element2.getAttribute("data-ce-tag") === "static") {
      blocks.push({ index: index2, node: { type: "html", value: "" }, verbatim: true });
      continue;
    }
    const node2 = blockFrom(element2);
    if (node2) {
      blocks.push({ index: index2, node: node2, verbatim: false });
    }
  }
  return blocks;
}
...
function serializeBlock(node2) {
  return toMarkdown(node2, OPTIONS).replace(/\n+$/, "");
}
function sameBlock(a, b) {
  return serializeBlock(collapseText(a)) === serializeBlock(collapseText(b));
}
const WHITESPACE = /[\t\n\r ]+/g;
function collapseText(node2) {
  if (node2.type === "text") {
    return { ...node2, value: String(node2.value).replace(WHITESPACE, " ") };
  }
  const children = node2.children;
  if (!Array.isArray(children)) {
    return node2;
  }
  return { ...node2, children: children.map(collapseText) };
}
const DEFAULT_GAP = "\n\n";
class MarkdownDocument {
   ...
   */
  update(html2, options) {
    const emitted = this.emit(fromHTML(html2));
   ...
   */
  gapAfterFrontmatter(body) {
    if (!body) {
      return "\n";
    }
    const front = this.parsed.frontmatter;
    return front && this.parsed.blocks.length ? this.parsed.source.slice(front.end, this.parsed.blocks[0].start) : DEFAULT_GAP;
  }
  ...
  join(emitted) {
    ...
    let text2 = emitted[0].text;
    for (let i = 1; i < emitted.length; i += 1) {
      const previous2 = emitted[i - 1];
      const current = emitted[i];
      const adjacent = previous2.index !== null && current.index !== null && current.index === previous2.index + 1;
      text2 += adjacent ? source.slice(blocks[previous2.index].end, blocks[current.index].start) : DEFAULT_GAP;
      text2 += current.text;
    }
    const last = emitted[emitted.length - 1];
    const endsAsBefore = last.index !== null && last.index === blocks.length - 1;
    return text2 + (endsAsBefore ? source.slice(blocks[last.index].end) : "\n");
  }
  /** The frontmatter block to write, or null if there is none. */
  frontmatterText(options) {
    const front = this.parsed.frontmatter;
...
export {
  ...
  MarkdownDocument as M,
  SOURCE_ATTRIBUTE as S,
  index as a,
  blockToHTML as b,
  fromHTML as f,
  isRepresentable as i,
  parseMarkdown as p,
  serializeBlock as s,
  toHTML as t
};
